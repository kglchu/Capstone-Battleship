mainGame.prototype.enemy = function()
{
	var myself = this;
	
	myself.Battleship.GameState.spawnPlayerBoard = function(board) {
	...
	myself.Battleship.GameState.miss = function(cell) {
	...
	myself.Battleship.GameState.sunkPlayerBattleship = function(cell) {
	...
	myself.Battleship.GameState.simulateShooting = function(cellGroup) {
		...
		if (this.reservedBullets > 0 && !this.game.data.loser) {
			...
			if(this.playerShipLastHit && this.playerShipFound) {
				...
				if (!cell.isHit) {
					...
					setTimeout(function () {
					myself.Battleship.GameState.unselectCell(cell);
					}, 350);
				} else {
			...
			setTimeout(function() {
				//myself.Battleship.game.data.isShooting = true;
				myself.Battleship.GameState.switchTurn('player');
			}, 100);
		}
	...
	function callShootAgain(cells) {
		myself.Battleship.GameState.simulateShooting(cells);
	}

	myself.Battleship.GameState.getAdjacentCell = function(focusedCell) {
	...
	myself.Battleship.GameState.checkTopCell = function(target) {
	...
	myself.Battleship.GameState.checkBottomCell = function(target) {
		var cell = this.playerCells.getChildAt(target + 1);
		if (cell) {
			return cell;
		} else {
	...
	myself.Battleship.GameState.checkRightCell = function(target) {
	...
	myself.Battleship.GameState.checkLeftCell = function(target) {
		if (target < 9 || target > 90) {
			return undefined;
		} else {
			var cell = this.playerCells.getChildAt(target - 10);
	...
	myself.Battleship.GameState.updateEnemyAmmo = function() {
	...
	myself.Battleship.GameState.checkEnemyCollision = function() {
		...
		// trigger explosion
		  ...
		  if (this.gameOver) {
			...
			this.music.stop();
			this.game.data.loser = "player";
			setTimeout(function(){myself.Battleship.game.state.start("GameOverState");}, 500);
		  }
	...
	myself.Battleship.GameState.GameOverEnemy = function() {
	  var board = {};
